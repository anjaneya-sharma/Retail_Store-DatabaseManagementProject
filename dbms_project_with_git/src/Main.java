                ...
                if (op == 1) {
                    ...
                    if (ans == 1) {
                        while (true) {
                            ...
                            System.out.println("7)Add items in cart");
                            System.out.println("8)Remove items from cart");
                            ...
                            } else if (opt == 7) {  // add items in cart , requires pid , quantity , individual price , cart_id from customer
                                while (true) {
                                    int prodid=0;
                                    int resp = -1;
                                    System.out.println("Enter the product id of the product you want to add OR ENTER 0 TO EXIT");
                                    resp = sc.nextInt();
                                    sc.nextLine();
                                    if (resp == 0) break;
                                    else {
                                       ...
                                       //check if already exists in cart product list , if does , then just increase the quantity , else add entry

                                        String q37 = "Select * FROM Cart_Product_List WHERE cp_pid=? ";
                                        PreparedStatement st37 = con.prepareStatement(q37);

                                        st37.setInt(1,prodid );

                                        ResultSet rs37 = st37.executeQuery();
                                        int ans37=0;
                                        while (rs37.next()) {
                                            if (rs37.getInt(1) != NULL) {
                                                ans37 = 1;
                                            }
                                        }
                                        st37.close();

                                        if(ans37==1){ // product already present
                                            //just update the quantity , and the price and the cart price
                                            int pricee=0;

                                            String q370 = "Select * FROM Product WHERE p_id=?";
                                            PreparedStatement st370 = con.prepareStatement(q370);

                                            st370.setInt(1,prodid );

                                            ResultSet rs370 = st370.executeQuery();

                                            while (rs370.next()) {
                                                pricee=rs370.getInt(3);
                                            }
                                            st370.close();

                                            int cart_idd=0;
                                            String q377 = "Select cus_cart_id FROM Customer WHERE cus_username=?";
                                            PreparedStatement st377 = con.prepareStatement(q377);

                                            st377.setString(1,usridc );

                                            ResultSet rs377 = st377.executeQuery();

                                            while (rs377.next()) {
                                                cart_idd=rs377.getInt(1);
                                            }
                                            st377.close();

                                            System.out.println("DEBUG 1 QUANT , PRICE , PRODID , CARTIDD "+quant+" "+pricee+" "+prodid+" "+cart_idd);

                                            String q371 = "UPDATE Cart_Product_List SET cp_quantity=cp_quantity+? , cp_price = cp_price + ? WHERE cp_pid=? AND cp_cart_id=?";
                                            PreparedStatement st371 = con.prepareStatement(q371);
                                            st371.setInt(1,quant );
                                            st371.setInt(2,pricee*quant );
                                            st371.setInt(3,prodid );
                                            st371.setInt(4,cart_idd );

                                            int rs371 = st371.executeUpdate();
                                            if(rs371==1){
                                                System.out.println("Cart updated successfully");
                                            }
                                            else{
                                                System.out.println("error encountered in updating cart");
                                            }
                                            st371.close();

                                            String q375 = "UPDATE Cart SET total_price= total_price +? WHERE cart_id=? ";
                                            PreparedStatement st375 = con.prepareStatement(q375);

                                            st375.setInt(1,quant*pricee );
                                            st375.setInt(2,cart_idd );

                                            int rs375 = st375.executeUpdate();
                                            if(rs375!=1) System.out.println("Error in updating  cart balance");

                                            st375.close();


                                        }
                                        else if(ans37!=1){// product not present , add a new entry
                                            //1)fetch price 2)fetch cart id
                                            int price=0;
                                            int cart_id=0;
                                            String q372 = "Select * FROM Product WHERE p_id=?";
                                            PreparedStatement st372 = con.prepareStatement(q372);

                                            st372.setInt(1,prodid );

                                            ResultSet rs372 = st372.executeQuery();

                                            while (rs372.next()) {
                                                price=rs372.getInt(3);
                                            }
                                            st372.close();

                                            String q373 = "Select * FROM Customer WHERE cus_username=?";
                                            ...
                                            while (rs373.next()) {
                                                cart_id=rs373.getInt(10);
                                            }
                                            st373.close();

                                            int finalPriceToAdd=quant*price;

                                            String q374 = "insert into Cart_Product_List (cp_quantity, cp_cart_id, cp_price, cp_pid,cp_cart_id) values (?,?,?,?,?);";
                                            PreparedStatement st374 = con.prepareStatement(q374);

                                            st374.setInt(1,quant );
                                            st374.setInt(2,cart_id );
                                            st374.setInt(3,finalPriceToAdd);
                                            st374.setInt(4,prodid);
                                            st374.setInt(5,cart_id);

                                            int rs374 = st374.executeUpdate();
                                            if(rs374==1) System.out.println("Product added to cart successfully");
                                            else {
                                                System.out.println("Error encountered in adding product to cart");
                                            }
                                            ...
                                            st375.setInt(1,quant*price );
                                            st375.setInt(2,cart_id );
