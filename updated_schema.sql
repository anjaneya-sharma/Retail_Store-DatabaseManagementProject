...
CREATE TABLE Cust_History(
 ...
 SET SQL_SAFE_UPDATES = 0;   -- REMEMBER TO TURN IT TO 1!!!!!!!!!!!!!!
...
delimiter //
create trigger wallet_checkk
before insert on Pays
for each row
if (select cus_wallet_balance from Customer where cus_id=pys_cus_id) < (select pt_amount from Payment where pt_id=pys_pt_id) then set
 ...
 FOR EACH ROW 
 UPDATE Customer
 SET cus_wallet=cus_wallet+100;
 
 CREATE TRIGGER add_to_wallet AFTER INSERT ON Customer
FOR EACH ROW
    UPDATE Customer
    SET cus_wallet = NEW.cus_wallet + 100 
...
DELIMITER ;
 ...
 CREATE TRIGGER testing_another_one
 AFTER INSERT ON Customer
 FOR EACH ROW 
 UPDATE Seller
 SET s_id=s_id+1000;
...
Select * FROM Seller;
 ...
 DROP TRIGGER wallet_update;
 DROP TRIGGER add_to_wallet;
...
Select * From Delivery_Partner WHERE dp_id =10;


SELECT * FROM Delivery_Partner  where dp_id=191919191919191919;

Select * FROM Places WHERE pl_cus_id=10;


Select * From Product where p_id =455;
...
show tables;
drop table is_added_to , Manages
